...
def play_files_parallel(td_dir, npy_files, save_dir, height, width):
    """
    Plays simultaneously files and their boxes in a rectangular format.
    """
    filelist_dirs = sorted(os.listdir(td_dir))
    ev_filelist = list()
    lb_filelist = list()
    ...
    for idx in range(len(ev_filelist)):
        ...
        for npz_num in range(len(events)):
            ...
            cv2.imwrite(save_dir + str(p_idx).zfill(4) + ".jpg", frame)
            cv2.waitKey(1)
...
def parse_args():
    ...
    parser.add_argument(
        '-t', "--td_dir",
        default="/home/wds/Desktop/exp_prophesee_still/test",
        type=str, help="input event files, annotation files are expected to be in the same folder")
    parser.add_argument(
        '-n', "--npy_file", default="/home/wds/Desktop/red_detector/det_result/prediction.npy", type=str,
        help="model predictions(bounding boxes), type [x_min, y_min, x_max, y_max, confidence, cls_id]")
    parser.add_argument("--save_dir", default="/home/wds/Desktop/detection_results/", type=str,
                        help="save images and transform to mp4")
...
if __name__ == "__main__":
    ARGS = parse_args()
    play_files_parallel(ARGS.td_dir, ARGS.npy_file, ARGS.save_dir, ARGS.height, ARGS.width)
