...
def play_files_parallel(event_dir, label_dir, height, width):
    """
    Plays simultaneously files and their boxes in a rectangular format.
    """
    ev_dir = sorted(os.listdir(event_dir))
    lb_dir = sorted(os.listdir(label_dir))

    event_list, label_list = [], []
    for ev, lb in zip(ev_dir, lb_dir):
        event_path = os.path.join(event_dir, ev)
        label_path = os.path.join(label_dir, lb)
        event_list.append(event_path), label_list.append(label_path)
...
def parse_args():
    ...
    parser.add_argument(
        '-r1', '--records1', type=str,
        default="/home/wds/Videos/prophesee_dlut/val/valfilelist01/events/moorea_2019-06-26_test_01_000_2501500000_2561500000_0",
        help='input event files, annotation files are expected to be in the same folder')
    parser.add_argument(
        '-r2', '--records2', type=str,
        default="/home/wds/Videos/prophesee_dlut/val/valfilelist01/labels/moorea_2019-06-26_test_01_000_2501500000_2561500000_0",
        help='input event files, annotation files are expected to be in the same folder')
...
if __name__ == '__main__':
    ARGS = parse_args()
    play_files_parallel(ARGS.records1, ARGS.records2, ARGS.height, ARGS.width)
