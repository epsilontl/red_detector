...
def play_files_parallel(root_dir, height, width):
    ...
    for idx in range(len(event_list)):
        ...
        boxes = rfn.structured_to_unstructured(boxes)[:, [1, 2, 3, 4, 5]]  # (x, y, w, h, class_id)
        boxes = cropToFrame(boxes, height, width)
        boxes = filter_boxes(boxes, 60, 20)  # filter boxes
        im = frame[0:height, 0:width]
        # call the visualization functions
        im = vis.make_binary_histo(events, img=im, width=width, height=height)
        draw_bboxes(im, boxes, labelmap=labelmap)
...
def parse_args():
    ...
    parser.add_argument(
        '-r', '--records', type=str,
        default="/home/wds/Desktop/exps1/train",
