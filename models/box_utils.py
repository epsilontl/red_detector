...
def point_form(boxes):
    ...
    :param boxes: (tensor), (center_x, center_y, width, height)
    :return boxes: (tensor), (x_min, y_min, x_max, y_max)
    """
...
def center_size(boxes):
    ...
    :param boxes: (tensor), (x_min, y_min, x_max, y_max)
    :return boxes: (tensor), (center_x, center_y, width, height)
    """
...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    :param threshold: (float) The overlap threshold used when mathing boxes.
    :param truths: (tensor) Ground truth boxes,
                   Shape: [num_obj, num_priors].
    :param priors: (tensor) Prior boxes from priorbox layers,
                   Shape: [n_priors,4].
    :param variances: (tensor) Variances corresponding to each prior coord,
                   Shape: [num_priors, 4].
    :param labels: (tensor) All the class labels for the image,
                   Shape: [num_obj].
...
def encode(matched, priors, variances):
    ...
    :param priors: (tensor) Prior boxes in center-offset form
                   Shape: [num_priors, 4].
    :param variances: (list[float]) Variances of prior boxes
...
def decode(loc, priors, variances):
    ...
    Note that default boxes and offset boxes are (x, y, w, h), while predicted boxes are (x_min, y_min, x_max, y_max)!
    :param loc: [num_priors, 4], (tensor) Loc preds from loc layers
    :param priors: [num_priors, 4], (tensor) Prior boxes and variances from priorbox layers
    :param variances: variances of prior boxes (list[float])
    :return boxes: [num_priors, 4], decoded bounding box predictions
    """
    boxes = torch.cat((
...
def log_sum_exp(x):
    ...
    :param x: (Variable(tensor)): conf_preds from conf layers
    """
    x_max = x.data.max()
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    :param boxes: (tensor) The location preds for the img,
                  Shape: [num_priors,4].
    :param scores: (tensor) The class predscores for the img,
                   Shape:[num_priors].
